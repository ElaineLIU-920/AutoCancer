...
def main():
    ...
    parser.add_argument('--num_top_gene_pair', type=int, default=50,
    ...
    if args.use_computed_attention:
        ...
        df_diag_sum = tri_attention(attn_matrix)
        gene_pari_attention_weight = top_gene_pair_att(df_diag_sum, num=args.num_top_gene_pair)

        print('-'*25)
        print('Top %d gene:' %(args.num_top_gene_pair))
        print('-'*25)
        print('Gene1   Gene2  Weight')
        print('-'*25)
        for i in gene_pari_attention_weight.index[:args.num_top_gene_pair]:
            print(gene_pari_attention_weight.Gene1.loc[i], gene_pari_attention_weight.Gene2.loc[i], gene_pari_attention_weight.Weight.loc[i])
        sys.exit()
